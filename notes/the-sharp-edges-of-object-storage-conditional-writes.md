...
The tradeoff we took was to disable retries in the storage lock clients: [apache/hudi#17869](https://github.com/apache/hudi/pull/17869). This did mean that the lock service needed its own SDK client with retry behavior configured separately from normal object-store traffic. Keeping retries at the lock protocol layer made the behavior easier to reason about: a `412` could no longer be caused by the client's own hidden retry.

However once lower-level retries were disabled, transient object-store failures surfaced directly in the lock code, leading to higher frequency of 5XXs and occasional latency spikes. One of those failures was specific to GCS: acquiring and releasing the same lock object in under a second could return `429`, because GCS documents a maximum write rate of [one write per second to the same object name](https://cloud.google.com/storage/quotas). With a single lock file, you can hit that limit even when the table itself is nowhere near an object-store throughput ceiling.
...
It is not enough to ask whether S3, GCS, and Azure all have some version of conditional writes. The details really matter! DigitalOcean Spaces (ceph-based) is a concrete example of this. It is S3-compatible, but that does not mean every [S3 concurrency primitive is there](https://docs.digitalocean.com/reference/api/spaces/). Its docs list conditional headers for reads like `GetObject` and `HeadObject`, but notably omit the `PutObject` behavior these new systems rely on. Therefore, both the lock protocol I built, and turbopuffer's distributed queue are not compatible with DigitalOcean Spaces at this time of writing. This is not because the underlying RADOS layer lacks atomic primitives, but I suspect that exposing fully correct S3-level conditional write semantics through the RGW metadata/indexing layer is complex.
...
Obviously chaos testing does not replace careful design. TLA+ is something I wish I had spent more time on. Retries belong at the protocol layer, not hidden inside a generic SDK client. Key layout matters tremendously. A single hot object is easy to reason about, but it is also a bottleneck and may run straight into provider limits.
...
I suspect compare-and-swap on object storage will become one of the foundational infrastructure primitives of the next decade. Locks, queues, WALs, manifests, cache validation, leader election, and state files are all starting to migrate onto object stores. As conditional writes become more standardized and strongly consistent across cloud providers, increasingly sophisticated distributed systems will be built directly atop S3-like storage, without needing a separate coordination layer for many workloads.